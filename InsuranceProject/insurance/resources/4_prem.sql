...
-- 3.4 保费参数表分析与开发过程
-- 步骤一: 计算是否在缴费期内ppp_、bpp_字段
create or replace  view  insurance_dw.prem_src1 as
select
    *,---- *号直接将prem_src0的所有字段都拿过来。
    if( policy_year <= ppp ,1,0 ) as ppp_,---如果保单年度<=缴费期间ppp，则ppp_=1，否则ppp_=0
    if( policy_year <= bpp ,1,0 ) as bpp_ --如果保单年度<=保险期间bpp，则bpp_=1，否则bpp_=0，（可以推断出如果是终身保障，则bpp_恒=1）
from insurance_dw.prem_src0;

-- 校验步骤一:--查看并验证prem_src1内容
select * from insurance_dw.prem_src1 where age_buy = 25 and sex = 'M' and ppp = 20;

-- 步骤二:  计算死亡率qx、kx、qx_ci字段
...
select *,
    cast(if(age = 105, qx - qx_ci, qx * (1-kx)  ) * bpp_ as decimal(17,12)) as qx_d
