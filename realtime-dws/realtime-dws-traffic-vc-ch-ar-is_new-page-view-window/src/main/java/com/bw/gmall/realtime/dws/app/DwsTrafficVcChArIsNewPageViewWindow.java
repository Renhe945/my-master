...
import com.bw.gmall.realtime.common.base.TrafficPageViewBean;
...
//        result.print();
    ...
    private void writeToDoris(SingleOutputStreamOperator<TrafficPageViewBean> result) {
        result.map(new DorisMapFunction<>()).sinkTo(FlinkSinkUtil.getDorisSink(Constant.DWS_TRAFFIC_VC_CH_AR_IS_NEW_PAGE_VIEW_WINDOW));
    }
    private SingleOutputStreamOperator<TrafficPageViewBean> windowAndAgg(SingleOutputStreamOperator<TrafficPageViewBean> beanStream) {
        return beanStream.assignTimestampsAndWatermarks(
                ...
                .keyBy(bean -> {
                    ...
                    String ch = bean.getCh();
                    String ar = bean.getAr();
                    String isNew = bean.getIsNew();
                    if (vc == null || ch == null || ar == null || isNew == null) {
                        System.err.println("分区键字段存在null值，当前bean: " + bean);
                        // 这里可以根据业务需求决定如何处理，比如赋予默认值或者直接跳过这条数据等，此处简单赋予默认值示例
                        vc = vc == null? "unknown_vc" : vc;
                        ch = ch == null? "unknown_ch" : ch;
                        ar = ar == null? "unknown_ar" : ar;
                        isNew = isNew == null? "unknown_isNew" : isNew;
                    }
                    return vc + "_" + ch + "_" + ar + "_" + isNew;
                })
        ...
        return stream.map(new MapFunction<String, JSONObject>() {
                    ...
                    public JSONObject map(String value) throws Exception {
                        try {
                            ...
                            System.err.println("解析JSON数据出现异常: " + e.getMessage() + ", 异常数据: " + value);
                            return null;
                        }
                    }
                })
                .filter(obj -> obj!= null) // 过滤掉解析失败的null数据
                .keyBy(obj -> {
                    try {
                        return obj.getJSONObject("common").getString("mid");
                    } catch (JSONException e) {
                        System.err.println("获取mid字段出现异常: " + e.getMessage() + ", 当前JSON对象: " + obj);
                        return "unknown_mid"; // 赋予默认值
                    }
                })//顺便计算下 uv, 按照 mid 进行分组, 找到同一个 mid 的第一条记录, uv 置为 1, 其他的uv为 0
                .process(new KeyedProcessFunction<String, JSONObject, TrafficPageViewBean>() {
                    ...
                    public void processElement(JSONObject obj,
                        ...
                        Long ts = null;
                        try {
                            page = obj.getJSONObject("page");
                            common = obj.getJSONObject("common");
                            ts = obj.getLong("ts");
                        } catch (JSONException e) {
                            System.err.println("获取page、common或ts字段出现异常: " + e.getMessage() + ", 当前JSON对象: " + obj);
                        }
                        if (page == null || common == null || ts == null) {
                            return; // 如果关键数据获取失败，直接跳过这条数据
                        }
                        ...
                        Long uv = 0L;
                        if (lastVisitDate!= null &&!today.equals(lastVisitDate)) {
                            // 是当天的第一次
                            uv = 1L;
                            // 更新状态
                        ...
                        try {
                            ...
                            System.err.println("设置TrafficPageViewBean的字段出现异常: " + e.getMessage() + ", 当前JSON对象: " + obj);
                        }
                        bean.setPvCt(pv);
                        bean.setUvCt(uv);
